...
async function fetchDatas(req , res){
  try{
    const response = await axios.get('http://192.34.63.158:4000/locations/getByState/Goa')
...
// console.log("data:" ,data?.exactLocation[0]?.locationDescription)
  ...
  return (
    <>
      ...
      <br />
      <br />
      {data.exactLocation && data.exactLocation.map((x) => 

<Link to={`/commentAndhra/${data.state}/${x.locationTitle}`}>
