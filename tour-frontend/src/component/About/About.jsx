...
const About = () => {
  ...
  const loadTeam = async () => {
    ...
    const { data } = await axios.post(`http://13.235.31.8:3008/about/team/getAll`,
    )
  ...
  const loadOffer = async () => {
    ...
    const { data } = await axios.post(`http://13.235.31.8:3008/about/offer/getAll`,
    )
    console.log(data.success, "jk");
    if (data.status === true) {
      console.log(data.data, "data")
      ...
      <div className="div1">
        ...
        {dataOffer && dataOffer.map((i) => (
          <div className="uarebest">
            <img src={`http://13.235.31.8:3008/offer/${i.logo}`} alt="" id="team" />
            <h4>{i.title}</h4>
            <p>{i.des}</p>
          </div>
        ))}
...
<div className="progress-bar">
      ...
      <div>
        ...
        <div className="OurTeam  div1">
          ...
          {data && data.map((i) => (
            <div className="uarebest">
              <img src={`http://13.235.31.8:3008/team/${i.logo}`} alt="" id="team" />
              <h4>{i.title}</h4>
              <p>{i.des}</p>
            </div>
          ))}
